...
from django_googleapi_app.mixins import(
	AjaxFormMixin, 
	reCAPTCHAValidation,
	FormErrors,
	RedirectParams,
...
def profile_view(request):
	...
	# replacing: if request.is_ajax(). Cos is_ajax() is deprecated
...
class SignUpView(AjaxFormMixin, FormView):
	...
	def get_context_data(self, **kwargs):
		...
		context["recaptcha_site_key"] = os.getenv('RECAPTCHA_PUBLIC_KEY')
		return context

	#over write the mixin logic to get, check and save reCAPTURE score
	def form_valid(self, form):
		response = super(AjaxFormMixin, self).form_valid(form)	
		# replacing: if self.request.is_ajax(). Cos is_ajax() is deprecated
